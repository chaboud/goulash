...
fn print_effective() -> i32 {
    ...
    let rows: Vec<(String, String)> = vec![
        ...
        ("engine.fast.watch".into(), e.fast.watch.to_string()),
        ("engine.fast.thinking".into(), e.fast.thinking.clone()),
        ("engine.slow.watch".into(), e.slow.watch.to_string()),
        ("engine.slow.ask".into(), e.slow.ask.to_string()),
        ("engine.slow.thinking".into(), e.slow.thinking.clone()),
        ("engine.slow.reasoning_tokens".into(), e.slow.reasoning_tokens.to_string()),
